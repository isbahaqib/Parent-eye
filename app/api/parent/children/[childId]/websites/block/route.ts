...
export async function PATCH(
  ...
  child.blockedWebsites = Array.from(new Set([...(child.blockedWebsites || []), website]));
  await writeDb(db);

  return NextResponse.json({ message: "Website blocked", blockedWebsites: child.blockedWebsites });
}
